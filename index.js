...
rcaa.encodingLength = function(data) {
  return (
    string.encodingLength(data.tag) + string.encodingLength(data.value) + 2
  );
};
...
rrrsig.encodingLength = function(sig) {
  return (
    20 + name.encodingLength(sig.signersName) + Buffer.byteLength(sig.signature)
  );
};
...
typebitmap.encode = function(typelist, buf, offset) {
  ...
  for (var i = 0; i < typelist.length; i++) {
    ...
    typesByWindow[typeid >> 8][(typeid >> 3) & 0x1f] |=
      1 << (7 - (typeid & 0x7));
  }
...
question.encode = function(q, buf, offset) {
  ...
  buf.writeUInt16BE(
    classes.toClass(q.class === undefined ? "IN" : q.class),
    offset
  );
  offset += 2;
